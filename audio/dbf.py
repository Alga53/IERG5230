...
output_file = f'output-{sample_file}-{direction}-xkc.wav'

# 1
# traj = np.load('Trajectory_1_600.npy')[:, 1:3]

traj_xkc = np.load('3_new_Trajectory_xkc.npy')[:, 1:3]
traj_dlr = np.load('3_new_Trajectory_dlr.npy')[:, 1:3]

# 4
# traj_xkc = np.load('4_Trajectory_0_xkc.npy')[:, 1:3]
# traj_dlr = np.load('4_Trajectory_1_dlr.npy')[:, 1:3]
traj_start_frame = 0
traj_end_frame = 600
